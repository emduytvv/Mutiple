...
public class TripleArrow : IShootStrategy
{
    ...
    public void Shoot(string prefabName, Vector3 spawnCenter, float angle180, float phys, float mag, float pen)
    {
        ...
        GameObject a1 = PhotonNetwork.Instantiate(prefabName, spawnCenter + dir * SpawnRadius, rot);
        GameObject a2 = PhotonNetwork.Instantiate(prefabName, spawnCenter + dir * SpawnRadius + perp * SideOffset, rot);
        GameObject a3 = PhotonNetwork.Instantiate(prefabName, spawnCenter + dir * SpawnRadius - perp * SideOffset, rot);
        ...
        a3.GetComponentInChildren<ArrowDamageSender>().SetDamage(phys, mag, pen);
    }
}
